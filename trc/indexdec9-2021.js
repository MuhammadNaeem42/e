...
var server = http.createServer(function(req, res) {
    ...
    req.on('end', function() {
        ...
        if(data.method == "createaddress")
        {
            const tronWeb = new TronWeb({
              fullHost: 'http://148.251.52.233:8091',
           });
        ...
        else if(data.method == "tokentransfer")
        {
            ...
            const fullNode = new HttpProvider("http://148.251.52.233:8091");
            const solidityNode = new HttpProvider("http://148.251.52.233:8091");
            const eventServer = new HttpProvider("http://148.251.52.233:8091");
    ...
    else if(data.method == "ercbalance")
    {
        ...
        const web3 = new Web3('http://148.251.52.233:8545');
    ...
    else if(data.method == "bnbbalance")
    {
        ...
        // Set up Infura as your RPC connection
        const web3 = new Web3('http://148.251.52.233:6545');
        //console.log("data",data.address);
        if(data.address != null && data.address != undefined && data.address != '0')
        {
           // console.log("data",data.address);
