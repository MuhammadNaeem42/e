...
class BinanceCoin_wallet_model extends CI_Model {  
	/*
	...
	*/
	...
	function eth_getBalance($address, $contract_address='', $block='latest', $decode_hex=FALSE)
	{
		...
		/*$get_trans_url = 'https://api.bscscan.com/api?module=account&action=balance&address='.$address.'&tag=latest';
		echo $get_trans_url;
		//exit;
		$ch = curl_init();
	...
	function listalltransactions1($user_trans_res, $contract_address='') 
	{
		...
		{	
			if(str_replace(" ","",$value['currency_name'])=='BinanceCoin'){

				echo $get_trans_url = 'https://api.bscscan.com/api?module=account&action=txlist&address='.$key;
				echo "<br>";
			}
		}
	}
	...
	function listalltransactions($coinname, $user_trans_res, $contract_address='') 
	{
		$address_list     = $user_trans_res['address_list'];
		$transactionIds   = $user_trans_res['transactionIds'];
		// echo "<pre>";print_r($address_list);die;
		...
		foreach ($address_list as $key => $value) 
		{
			if(str_replace(" ","",$value['currency_name'])=='BinanceCoin'){
				...
				// $get_trans_url = 'https://testnet.bscscan.com/api?module=account&action=txlist&address='.$key;
				...
				{ //echo 'HERE';
					foreach ($trans->result as $trans_key => $trans_value) 
					{
						...
						$trans_arr = array('account'		 => $acc_owner,
								            'address'		 => $acc_address,
								            'category'		 => $cat_sat,
								            'amount'		 => $rec_amount,
								            'blockhash'		 => $blockHash,
