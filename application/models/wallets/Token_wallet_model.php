...
class Token_wallet_model extends CI_Model {  
	/*
	...
	*/
	...
	public function __construct() 
	{
		...
		if(!empty($wallet_row)){
			...
			$this->wallet_port 	= decryptIt($wallet_row->portnumber);
			$this->wallet_ip   	= decryptIt($wallet_row->allow_ip); 
			$this->version	   	=  "2.0";
			$this->username   	= $wallet_username;
			$this->password   	= $wallet_password;
    ...
    $ch = curl_init();
        ...
        curl_close($ch);
        $Val_Bal = $this->decode_hex($resp['result']['_hex']);

        return $Val_Bal / $decimal_places;
	} 
...
//         $Currency_Sym = current($Arr_Unique).'-ERC20';
...
function listalltransactions($user_trans_res, $contract_address='') 
	{
		...
		foreach ($address_list as $key => $value) {
			...
			$get_trans_url = 'https://api.etherscan.io/api?module=account&action=tokentx&contractaddress='.$contract_address.'&address='.$address.'&page=1&offset=100&sort=asc&apikey=9CCE5UWF269JBWP89P2HTQS84VY2EUHYV5';

			//echo $get_trans_url."<br>";
...
if($ret1 !== FALSE)
		...
		exit();*/
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'http://'.$this->wallet_ip.":".$this->wallet_port);
		// curl_setopt($ch, CURLOPT_URL, 'http://138.201.56.226:8545');
