<?php
 ...
 class Trading_fee extends CI_Controller {
 	...
 	function tradingfee_ajax()
 	{
        ...
        $valid_columns = array(
            ...
            4=>'taker_fee',
            5=>'withdrawfee',
            6=>'minimum_withdrawfee',
            7=>'maximum_withdrwafee',
            8=>'sort_order',
            9=>'status',
            10=>'status'
        );
        ...
        if(!empty($search))
        { 
            $like = "WHERE (blackcube_tradingfee.currency_name LIKE '%".$search."%' OR blackcube_tradingfee.currency_symbol LIKE '%".$search."%')"; 
        }
        
        $query = "SELECT * FROM `blackcube_tradingfee` ".$like." ORDER BY id DESC LIMIT ".$start.",".$length;
		...
		if(count($users_history_result)>0)
		{
			foreach($users_history->result() as $users){
				...
				if ($users->status == 1) {
                            ...
                            $extra = array("data-placement"=>"bottom",'data-toggle'=>"popover","data-content"=>"De-activate this trading fee","class"=>"poper");
                            $changeStatus = anchor(admin_url().'trading_fee/status/' . $users->id . '/0','<i class="fa fa-unlock text-primary"></i>',$extra);
                        } else {
                            $status = '<label class="label label-danger">De-Activated</label>';
                            $extra = array("data-placement"=>"bottom",'data-toggle'=>"popover","data-content"=>"Activate this trading fee","class"=>"poper");
                            $changeStatus = anchor(admin_url().'trading_fee/status/' . $users->id . '/1','<i class="fa fa-lock text-primary"></i>',$extra);
                        }

                    $edit = '&nbsp;&nbsp;&nbsp;<a href="' . admin_url() . 'trading_fee/edit/' . $users->id . '" data-placement="top" data-toggle="popover" data-content="Edit this trading fee" class="poper"><i class="fa fa-pencil text-primary"></i></a>&nbsp;&nbsp;&nbsp;';
                    $link="'".admin_url().'trading_fee/delete/'.$users->id."'";
                    
                    $delete = '<a onclick="deleteaction('.$link.');" data-placement="bottom" data-toggle="popover" data-content="If you delete the trading fee? you cant revert back." class="poper"
...
$sort = '<a href="javascript:void(0);" title="Double Click here to change the order." class="sorting_my editable editable-click" data-pk="'.$users->id.'">'.$users->sort_order.'</a>';
				...
				$data[] = array(
						...
						$users->taker_fee,
						$users->withdrawfee,
						$users->minimum_withdrawfee,
						$users->maximum_withdrwafee,
						$sort,
						$status,
 	...
 	// 	$update = $this->common_model->updateTableData('tradingfee',array('id'=>$currency_id),array('sort_order'=>$sortorder));
 	// 	if($update)
 	// 	{
 	// 		echo true;
 	// 	}
	...
	function index() {
		...
		// Get the list pages
		$data['tradingfee'] = $this->common_model->getTableData('tradingfee', '', '', '', '', '', '', '', array('id', 'DESC'));
		$data['title'] = 'Trading Fee Management';
		$data['meta_keywords'] = 'Trading Fee Management';
		$data['meta_description'] = 'Trading Fee Management';
		$data['main_content'] = 'trading_fee/trading_fee';
		$data['view'] = 'view_all';
	...
	function edit($id) {
		...
		if ($id == '') {
			...
			admin_redirect('tradingfee');
		}
		$isValid = $this->common_model->getTableData('tradingfee', array('id' => $id));
		...
		$this->form_validation->set_rules('withdrawfee', 'Withdraw Fees', 'required|xss_clean');
		...
		if ($this->input->post()) {				
			...
			if ($this->form_validation->run())
			{
				...
				if($image!="") {
					...
					{
						$this->session->set_flashdata('error','Problem with your currency image');
						admin_redirect('trading_fee/edit/' . $id, 'refresh');
					}
				} else {
				...
				$currency_name=$this->input->post('currency_name');
				$currency_symbol=$this->input->post('currency_symbol');
				$details=$this->common_model->getTableData('tradingfee',array('currency_name'=>$currency_name),'','','','');//array('currency_symbol'=>$currency_symbol)
				...
				if($details->num_rows()==0||$details->row('id')==$id)
				{
					...
					$updateData['withdrawfee'] = $this->input->post('withdrawfee');
					...
					$updateData['minimum_withdrawfee'] = $this->input->post('min_withdraw_limit');
					$updateData['maximum_withdrwafee'] = $this->input->post('max_withdraw_limit');
					// if($this->input->post('currency_type')=='fiat'){
					// $updateData['deposit_fees'] = $this->input->post('deposit_fees');
					// $updateData['deposit_fees_type'] = $this->input->post('deposit_fees_type');
					// $updateData['min_deposit_limit'] = $this->input->post('min_deposit_limit');
     ...
     //                   $updateData['asset_type'] = 1;
					...
					//echo $this->db->last_query();die;
					$update = $this->common_model->updateTableData('tradingfee', $condition, $updateData);

					if ($update) {
						$this->session->set_flashdata('success', 'Trading Fee has been updated successfully!');
						admin_redirect('trading_fee', 'refresh');
					} else {
						$this->session->set_flashdata('error', 'Unable to update this tradingfee');
						admin_redirect('trading_fee/edit/' . $id, 'refresh');
					}
				}
				else
		...
		$data['tradingfee'] = $isValid->row();
		$data['action'] = admin_url() . 'trading_fee/edit/' . $id;
		$data['title'] = 'Edit Trading Fee';
		$data['meta_keywords'] = 'Edit Trading Fee';
		$data['meta_description'] = 'Edit Trading Fee';
		$data['main_content'] = 'trading_fee/trading_fee';
		$data['view'] = 'edit';
		$this->load->view('administrator/admin_template', $data);
	}
	...
	function status($id,$status) {
		...
		if ($id == '' || $status == '') { 
			$this->session->set_flashdata('error', 'Invalid request');
			admin_redirect('trading_fee');
		}
		$isValid = $this->common_model->getTableData('tradingfee', array('id' => $id))->num_rows();
		if ($isValid > 0) { // Check is valid banner 
			...
			if ($update) { // True // Update success
				if ($status == 1) {
					$this->session->set_flashdata('success', 'Trading Fee activated successfully');
				} else {
					$this->session->set_flashdata('success', 'Trading Fee de-activated successfully');
				}
				admin_redirect('trading_fee');
			} else { //False
				$this->session->set_flashdata('error', 'Problem occure with Trading Fee status updation');
				admin_redirect('trading_fee');	
			}
		} else {
			$this->session->set_flashdata('error', 'Unable to find this Trading Fee');
			admin_redirect('trading_fee');
		}
	}
	// Delete page
		...
		if ($isValid > 0) { // Check is valid 
			$condition = array('id' => $id);
			$delete = $this->common_model->deleteTableData('tradingfee', $condition);
